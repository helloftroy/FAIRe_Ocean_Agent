...
# true under sbatch) -- without this, a long-running job's own periodic
# progress logging can sit in that buffer for its entire runtime with
# nothing ever reaching the .out file. Every cluster/*.sbatch script also
# sets this directly (belt and suspenders, and effective immediately
# without needing to regenerate this file), but it belongs here too as
# the one place every job's environment is actually assembled.
export PYTHONUNBUFFERED=1
# Confirmed live: this cluster's Lustre-backed scratch mount cannot do the
# POSIX shared-memory-mapped file locking SQLite WAL mode needs, and can
# even fail a later PLAIN READ with the same locking error once WAL was
# attempted and failed. Skip the attempt outright instead of try-then-
# catch. See database/session.py's own comment on this env var.
...
# See the conda branch's own identical lines above for why.
export PYTHONUNBUFFERED=1
export FAIR_OCEAN_SKIP_SQLITE_WAL=1
_expected_prefix="${EXPECTED_PREFIX}"
_actual_prefix="\$(python -c 'import sys; print(sys.prefix)')"
