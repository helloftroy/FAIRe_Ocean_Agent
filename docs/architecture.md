...
| 27: Within-study sample-entity alias reconciliation (supplement naming vs. real accession) | Done -- direct follow-up after the user reviewed a real audit export (`two_remaining_current_pipeline_20260806_172744`) for s42003 and noticed `sampleMetadata.csv` carried two un-reconciled representations of the same physical samples: 67 rows named `GC04_1`..`GC05_19` (the paper's own native sample naming, materialized by `sources/supplement_parsing.py` from a supplement table's own sample-ID column) and, separately, 83 rows with real NCBI BioSample accessions -- never reconciled because entity matching is strict string equality and `"GC04_1"` never equals `"SAMN11268098"`. Investigated directly against the real database and found a 100%-reliable (67/67) cross-reference: each real BioSample's own `source_material_id`-family attribute already encodes the paper's native name plus a cruise-ID prefix (e.g. `SAMN11268098`'s value is `"GS14-GC08-1"`), matchable via a general (not paper-specific) tokenized trailing-subsequence match, not a regex hardcoded to this one paper's cruise-prefix convention. Also confirmed, and fixed, a related pre-existing bug sharing the same root cause: the real BioSample XML stores this attribute as `"source-material-id"` (hyphenated), not the literal `"source_material_id"` `sources/ncbi.py`'s own depth-derivation lookup used, so depth derivation silently never fired for this real dataset -- fixed with a case/separator-insensitive attribute-name lookup (`_get_attribute`). Chose a non-destructive, export-time merge over a real Entity-level FK merge, for three concrete reasons: discovery/supplement pipeline stages aren't auto-chained so a creation-time merge hook can't assume ordering; `mapping/faire.py`'s existing per-field conflict handling ("oldest fact wins, drop the rest") wouldn't produce the user's requested pipe-join regardless of where the two entities live; and the matching evidence is `DETERMINISTICALLY_DERIVED`, the same tier this codebase's own conventions already say must never merge unreviewed even at the Study level, with no un-merge mechanism in the schema for a false positive. New `identity/sample_alias_reconciliation.py::reconcile_sample_aliases` (called from `map_study_to_faire`, idempotent/self-healing on rerun): exactly-one trailing-subsequence match records a new `EntityRelationshipType.SAME_PHYSICAL_SAMPLE_AS` relationship (alias -> canonical) plus an audit `RawFact`; two or more matches never guesses, flagging `NEEDS_REVIEW` instead (mirroring `identity/root_determination.py::_flag_ambiguous`'s shape); zero matches is a silent no-op. Dedups by distinct target `entity_id`, not by fact row, after confirming live that a shared canonical entity carries one duplicate `source_material_id` RawFact per linked study's own resolution pass. `exports/faire.py` folds each alias into its canonical entity's row (new `_alias_entities`/`_merge_field_values` helpers): identical values collapse, a value on only one side passes through, genuine conflicts pipe-join (canonical's value first); `samp_name`/`materialSampleID` are excluded from merging (the real accession always wins as the identifier) and the alias's own native name(s) are preserved in a new internal-only `internal_alias_sample_ids` column; `experimentRunMetadata`'s `samp_name` resolution now also prefers a run's sample's canonical target, if any. Live re-check against a scratch copy of the real audit database confirmed the design's own predicted numbers exactly: `reconcile_sample_aliases` found 67 matches/0 ambiguous for s42003 (68 alias candidates considered), `sampleMetadata.csv` rows for that study dropped from 150 to 83 exactly as predicted, the `GC08_1`/`SAMN11268098` pair collapsed into one row (`samp_name="SAMN11268098"`, `internal_alias_sample_ids="GC08_1"`), zero ambiguous flags anywhere, and the unrelated fmicb study (a different, pure-depth-style `source_material_id` convention) produced zero alias relationships, confirming no false positives. 710 tests pass (8 new). See README's "Within-study sample-entity alias reconciliation". |
